...
function(modm_target_config_create target target_arch target_options target_warnings)
  ...
  set(LINKFLAGS
    ...
    -L${CMAKE_SOURCE_DIR}
    -nostartfiles
    -Tsrc/modm_app/modm/link/linkerscript.ld
    -Wl,--build-id=sha1
    -Wl,--fatal-warnings
    -Wl,--gc-sections
...
function(modm_targets_create project_name)
  ...
  add_custom_command(TARGET ${project_name}
    POST_BUILD
    COMMAND cmake -E env PYTHONPATH=${PROJECT_SOURCE_DIR}/modm ${Python3_EXECUTABLE} -m modm_tools.size ${project_name}.elf \"[{'name': 'itcm', 'access': 'rwx', 'start': 0, 'size': 16384}, {'name': 'flash', 'access': 'rx', 'start': 2097152, 'size': 2097152}, {'name': 'dtcm', 'access': 'rwx', 'start': 536870912, 'size': 131072}, {'name': 'sram1', 'access': 'rwx', 'start': 537001984, 'size': 376832}, {'name': 'sram2', 'access': 'rwx', 'start': 537378816, 'size': 16384}, {'name': 'backup', 'access': 'rwx', 'start': 1073889280, 'size': 4096}]\")
  add_custom_target(size DEPENDS ${project_name}.elf)
  add_custom_command(TARGET size
    USES_TERMINAL
    COMMAND cmake -E env PYTHONPATH=modm ${Python3_EXECUTABLE} -m modm_tools.size ${PROJECT_BINARY_DIR}/src/${project_name}.elf \"[{'name': 'itcm', 'access': 'rwx', 'start': 0, 'size': 16384}, {'name': 'flash', 'access': 'rx', 'start': 2097152, 'size': 2097152}, {'name': 'dtcm', 'access': 'rwx', 'start': 536870912, 'size': 131072}, {'name': 'sram1', 'access': 'rwx', 'start': 537001984, 'size': 376832}, {'name': 'sram2', 'access': 'rwx', 'start': 537378816, 'size': 16384}, {'name': 'backup', 'access': 'rwx', 'start': 1073889280, 'size': 4096}]\"
  ...
  add_custom_command(TARGET program-jlink
    USES_TERMINAL
    COMMAND cmake -E env PYTHONPATH=modm ${Python3_EXECUTABLE} -m modm_tools.jlink -device stm32f767zi
        ${PROJECT_BINARY_DIR}/src/${project_name}.elf
    WORKING_DIRECTORY ${PROJECT_SOURCE_DIR})
  ...
  add_custom_command(TARGET reset-jlink
    USES_TERMINAL
    COMMAND cmake -E env PYTHONPATH=modm ${Python3_EXECUTABLE} -m modm_tools.jlink -device stm32f767zi --reset
    WORKING_DIRECTORY ${PROJECT_SOURCE_DIR})
  ...
  add_custom_command(TARGET log-itm-jlink
    USES_TERMINAL
    COMMAND cmake -E env PYTHONPATH=modm ${Python3_EXECUTABLE} -m modm_tools.itm jlink -device stm32f767zi
    WORKING_DIRECTORY ${PROJECT_SOURCE_DIR})
  ...
  add_custom_command(TARGET log-rtt-jlink
    USES_TERMINAL
    COMMAND cmake -E env PYTHONPATH=modm ${Python3_EXECUTABLE} -m modm_tools.rtt --channel ${MODM_RTT_CHANNEL} jlink -device stm32f767zi
    WORKING_DIRECTORY ${PROJECT_SOURCE_DIR})
endfunction()
