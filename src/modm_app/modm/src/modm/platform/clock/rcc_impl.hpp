...
Rcc::enable()
{
	...
	if constexpr (peripheral == Peripheral::Can3)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->APB1ENR |= RCC_APB1ENR_CAN3EN; __DSB();
			RCC->APB1RSTR |= RCC_APB1RSTR_CAN3RST; __DSB();
	...
	if constexpr (peripheral == Peripheral::I2c4)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->APB1ENR |= RCC_APB1ENR_I2C4EN; __DSB();
			RCC->APB1RSTR |= RCC_APB1RSTR_I2C4RST; __DSB();
			RCC->APB1RSTR &= ~RCC_APB1RSTR_I2C4RST;
		}
	if constexpr (peripheral == Peripheral::Jpeg)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->AHB2ENR |= RCC_AHB2ENR_JPEGEN; __DSB();
			RCC->AHB2RSTR |= RCC_AHB2RSTR_JPEGRST; __DSB();
			RCC->AHB2RSTR &= ~RCC_AHB2RSTR_JPEGRST;
		}
	if constexpr (peripheral == Peripheral::Lptim1)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->APB1ENR |= RCC_APB1ENR_LPTIM1EN; __DSB();
			RCC->APB1RSTR |= RCC_APB1RSTR_LPTIM1RST; __DSB();
			RCC->APB1RSTR &= ~RCC_APB1RSTR_LPTIM1RST;
		}
	if constexpr (peripheral == Peripheral::Ltdc)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->APB2ENR |= RCC_APB2ENR_LTDCEN; __DSB();
			RCC->APB2RSTR |= RCC_APB2RSTR_LTDCRST; __DSB();
	...
	if constexpr (peripheral == Peripheral::Sdmmc2)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->APB2ENR |= RCC_APB2ENR_SDMMC2EN; __DSB();
			RCC->APB2RSTR |= RCC_APB2RSTR_SDMMC2RST; __DSB();
...
Rcc::isEnabled()
{
	...
	if constexpr (peripheral == Peripheral::Can3)
		return RCC->APB1ENR & RCC_APB1ENR_CAN3EN;
	if constexpr (peripheral == Peripheral::Crc)
	...
	if constexpr (peripheral == Peripheral::I2c4)
		return RCC->APB1ENR & RCC_APB1ENR_I2C4EN;
	if constexpr (peripheral == Peripheral::Jpeg)
		return RCC->AHB2ENR & RCC_AHB2ENR_JPEGEN;
	if constexpr (peripheral == Peripheral::Lptim1)
		return RCC->APB1ENR & RCC_APB1ENR_LPTIM1EN;
	if constexpr (peripheral == Peripheral::Ltdc)
		return RCC->APB2ENR & RCC_APB2ENR_LTDCEN;
	if constexpr (peripheral == Peripheral::Rng)
	...
	if constexpr (peripheral == Peripheral::Sdmmc2)
		return RCC->APB2ENR & RCC_APB2ENR_SDMMC2EN;
	if constexpr (peripheral == Peripheral::Spdifrx)
